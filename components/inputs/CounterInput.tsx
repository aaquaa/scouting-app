...
export default function CounterInput(data: CounterInputProps) {
  ...
  return (
    <div className="my-2 flex flex-row items-center justify-center">
      <button
        className="focus:shadow-outline w-8 rounded bg-gray-500 text-2xl text-white hover:bg-blue-700 focus:outline-none"
        type="button"
        onClick={() => handleChange(-(data.step || 1))}
      >
        -
      </button>
      <h2 className="px-4 text-2xl dark:text-black">{data.value}</h2>
      <button
        className="focus:shadow-outline w-8 rounded bg-gray-500 text-2xl  text-white hover:bg-blue-700 focus:outline-none"
        type="button"
        onClick={() => handleChange(data.step || 1)}
